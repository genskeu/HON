<template>
  <div
    class="relative grid grid-rows-6 grid-cols-1"
    @cornerstoneimagerendered.capture="displayStackIndex"
  >
    <!-- image viewer -->
    <div
      ref="viewer"
      class="dicom_viewer row-span-5 col-span-1 relative"
    >
      <!-- metadata viewer -->
...
export default {
  ...
  methods: {
    ...
    loadMaskOnSelect(event) {
      ...
      if(segMaskName){
        axios.get(url).then((response) => {
          var seg_data = response.data.maskData
    ...
    loadStackOnSelect(event) {
      const stack_name = event.target.value;
      if (stack_name != "") {
        const data = this.$store.getters["input/getStackData"](stack_name);
        ...
        this.loadDisplayCornerstone(stacks[0], stacks[1], "input").then(() => {
          ...
          this.filterMasks(stack_name);
        });
      }
    },
    filterMasks(volName) {
      var mask_select = this.$refs.mask_select_id;
      mask_select.value = "";
      for (var option of mask_select) {
        if (
    ...
    parseImagesCornerstone(images) {
      ...
      const series = [];
      for (let image of images.sort()) {
        series.push(image.split("/").pop());
      }
      const imageIds = series.map(
        (seriesImage) =>
          `${scheme}:${baseUrl}/uploads/${data_type}/${folder}/${seriesImage}`
      );
      const stack = {
